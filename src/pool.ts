import {
  ...
  PoolApproved as PoolApprovedEvent,
  AdminWithdrawal as AdminWithdrawalEvent,
...
export function handlePoolApproved(event: PoolApprovedEvent): void {
  let pool = Pool.load(event.address.toHexString());
  if (pool) {
    pool.approved = true;
    pool.save();
...
export function handleFundsTaken(event: FundsTakenEvent): void {
  ...
  if (pool) {
    ...
    // The lockupStartTime is set when admin takes funds (starts lock-up period)
    pool.lockupStartTime = event.block.timestamp;
    pool.save();
...
export function handleAdminWithdrawal(event: AdminWithdrawalEvent): void {
  // This event is emitted when admin withdraws funds during lock-up period
  // We can track this for analytics if needed, but for now we just log it
  // The pool state doesn't need to change as funds are still locked up
}
...
function updateCFMonthlyData(
  ...
  let monthTimestamp = timestamp
    .div(BigInt.fromI32(2629743))
    .times(BigInt.fromI32(2629743));
